...
export const getAllTechnicians = async (req, res) => {
    try {
        ...
        const data = technicians.map((tech) => {
            ...
            const todayAttendance = techObj.attendance?.find((att) => {
...
export const markAttendance = async (req, res) => {
    try {
        ...
        if (!technician.attendance)
            technician.attendance = [];
        const index = technician.attendance.findIndex((att) => {
            const d = new Date(att.date).getTime();
            return d >= startOfDay && d <= endOfDay;
        });
...
export const getAttendanceByMonth = async (req, res) => {
    try {
        ...
        const filtered = technician.attendance?.filter((att) => {
