...
export class TrackingRepository {
    ...
    async updateTracking(trackingId, data) {
        ...
        const tracking = await Tracking.findByIdAndUpdate(trackingId, updateData, { new: true, runValidators: true }).populate('technicianId', 'fullName email contactNumber');
        return tracking;
    }
    /**
     * Get latest tracking for a technician
     */
    async getLatestByTechnician(technicianId) {
     ...
     */
    async getNearbyTracking(longitude, latitude, radius = 5000) {
     ...
     * Get active technicians (with recent tracking)
     */
    async getActiveTechnicians(minutesAgo = 30) {
