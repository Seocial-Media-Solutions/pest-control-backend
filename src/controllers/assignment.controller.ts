...
const handleBookingStatusUpdate = async (assignment: any) => {
...
export const updateServiceStatus = async (req: Request, res: Response) => {
    try {
        ...
        // Manually find the sub-service object
        const subService = booking.subServiceIds.find((item: any) => {
            ...
            if (itemId && itemId.length > 24 && itemId.includes('"')) itemId = itemId.replace(/['"]+/g, '');
...
export const getAssignmentsByTechnician = async (req: Request, res: Response) => {
    try {
...
export const manageTreatmentPreparation = async (req: Request, res: Response) => {
    try {
...
export const addSitePicture = async (req: Request, res: Response) => {
    try {
        ...
        if (!assignment.applyTreatment) assignment.applyTreatment = { sitePictures: [] };
